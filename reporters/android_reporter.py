...
from executors.android_executor import AndroidTestSuiteResult, AndroidTestStatus


class AndroidTestReporter:
    """Android测试报告生成器"""
...
| **Android版本** | {result.android_version} |
| **应用包名** | {result.package_name or "N/A"} |
...
"""
        ...
        # 状态图标
        status_icons = {
            AndroidTestStatus.PASSED: "✅",
            AndroidTestStatus.FAILED: "❌",
            AndroidTestStatus.ERROR: "⚠️",
            AndroidTestStatus.SKIPPED: "⏭️",
        }
        ...
        # 失败用例详情
        failed_results = [r for r in result.results if r.status in [AndroidTestStatus.FAILED, AndroidTestStatus.ERROR]]
        if failed_results:
            report += "\n## 失败用例分析\n\n"
...
- **失败类型**: {"断言失败" if r.status == AndroidTestStatus.FAILED else "执行错误"}
...
"""
        ...
        for r in result.results:
            ...
            if r.status == AndroidTestStatus.PASSED:
                class_stats[r.test_class]["passed"] += 1
            elif r.status in [AndroidTestStatus.FAILED, AndroidTestStatus.ERROR]:
...
"""
    ...
    def _generate_json_report(self, result: AndroidTestSuiteResult) -> str:
        """生成JSON格式报告"""
...
<head>
    ...
    <title>Android测试报告 - {result.suite_name}</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: linear-gradient(135deg, #3ddc84 0%, #1a73e8 100%); min-height: 100vh; padding: 20px; }}
        ...
        .stat-item.passed .stat-value {{ color: #3ddc84; }}
        .stat-item.failed .stat-value {{ color: #ea4335; }}
        .stat-item.rate .stat-value {{ color: #1a73e8; }}
        ...
        .progress-ring .progress {{ stroke: #3ddc84; stroke-linecap: round; transform: rotate(-90deg); transform-origin: 50% 50%; }}
        ...
        .status-passed .status-dot {{ background: #3ddc84; }}
        .status-failed .status-dot {{ background: #ea4335; }}
        .status-error .status-dot {{ background: #fbbc04; }}
        .failure-card {{ border-left: 4px solid #ea4335; padding-left: 16px; margin: 16px 0; }}
        .failure-card h4 {{ color: #ea4335; margin-bottom: 8px; }}
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="device-info">
                ...
                <span class="device-badge">Android {result.android_version}</span>
...
"""
        ...
        for r in result.results:
            ...
            status_text = "通过" if r.status == AndroidTestStatus.PASSED else "失败" if r.status == AndroidTestStatus.FAILED else "错误"
...
"""
        ...
        if failed_results:
            html += """
        <div class="card">
            <h3 style="margin-bottom: 16px; color: #ea4335;">⚠️ 失败用例详情</h3>
"""
            for r in failed_results:
                html += f"""
...
"""
    ...
    def _generate_ai_analysis(self, result: AndroidTestSuiteResult) -> str:
        """使用AI生成分析报告"""
        if not self.llm_client:
            return ""

        config = get_prompt_config(PromptType.ANDROID_TEST_REPORT)

        # 准备测试结果摘要
        summary = {
            "suite_name": result.suite_name,
            "device": f"{result.device_name} (Android {result.android_version})",
            "total": result.total,
            ...
            "failed_cases": [
                ...
                for r in result.results
                if r.status in [AndroidTestStatus.FAILED, AndroidTestStatus.ERROR]
            ]
        }

        messages = PromptManager.build_messages(
            prompt_type=PromptType.ERROR_ANALYSIS,
            user_input="请分析以下Android测试失败原因并给出改进建议",
        ...
        try:
            analysis = self.llm_client.chat(
                ...
                temperature=config.temperature if config else 0.5
            )
            return analysis
    ...
    def save_report(
        ...
        if not filename:
            ...
            filename = f"android_report_{timestamp}"
